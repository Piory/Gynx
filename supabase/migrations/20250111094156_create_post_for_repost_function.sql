...
BEGIN
    BEGIN
        -- 投稿を作成
        INSERT INTO t_posts (id, user_id, referenced_post_id) VALUES (post_id, current_user_id, referenced_post_id);
        -- 自分のタイムラインに投稿を追加
        INSERT INTO t_user_timelines (user_id, type, post_id) VALUES (current_user_id, timeline_type, post_id);
        -- フォロワーのタイムラインにも投稿を追加
        SELECT COUNT(*) FROM t_user_follows WHERE follow_user_id = current_user_id INTO follow_count;
