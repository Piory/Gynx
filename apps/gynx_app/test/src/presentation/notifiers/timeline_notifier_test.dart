...
void main() {
  ...
  group('正常系', () {
    test('FetchPostUseCase#execute が呼ばれること', () async {
      final result =
          await createContainer().read(timelineNotifierProvider.future);
    ...
    group('正常系', () {
      test(
        ...
        () async {
          ...
          when(mockFetchPostUseCase.execute())
      ...
      test('すでに取得済みの投稿がある場合、取得済みの投稿の中で一番新しい投稿より新しい投稿を取得すること', () async {
        ...
        when(mockFetchPostUseCase.execute(sincePostId: 2))
            .thenAnswer((_) async => VPostList([vPost]));
        await container.read(timelineNotifierProvider.notifier).fetchNext();
  ...
  group('#fetchPrevious', () {
    group('正常系', () {
      ...
      test('すでに取得済みの投稿がある場合、取得済みの投稿の中で一番古い投稿より古い投稿を取得すること', () async {
        ...
        when(mockFetchPostUseCase.execute(maxPostId: 1))
            .thenAnswer((_) async => VPostList([vPost]));
  ...
  group('#maxPostId', () {
    group('正常系', () {
      test('一番値の大きい PostId が返ってくること', () async {
        ...
        final maxPostId =
            container.read(timelineNotifierProvider.notifier).maxPostId;
        expect(maxPostId, 2);
      });
    });
  });
  ...
  group('#minPostId', () {
    group('正常系', () {
      test('一番値の小さい PostId が返ってくること', () async {
        ...
        final minPostId =
            container.read(timelineNotifierProvider.notifier).minPostId;
        expect(minPostId, 1);
      });
    });
  });
