...
void main() {
  ...
  const url4 = 'https://example.com/image4.jpg';
  ...
  group('MediaForFour', () {
    ...
    //
    group('正常系', () {
      testWidgets('Media が4つ表示されていること', (tester) async {
        await pumpWidget(
          tester: tester,
          onTap: null,
        );
        expect(find.byType(Media), findsNWidgets(4));
      });
      ...
      testWidgets('4つ目の Media をタップしたら、$url4 が渡されること', (tester) async {
        var calledCount = 0;
        await pumpWidget(
          tester: tester,
          onTap: (actual) {
        ...
        expect(calledCount, 0);
        await tester.tap(find.byType(Media).last);
        expect(calledCount, 1);
      });
    });
  });
