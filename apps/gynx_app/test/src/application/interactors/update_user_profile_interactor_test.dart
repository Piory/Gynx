...
void main() {
  ...
  group('正常系', () {
    ...
    test('すべての項目の更新できること', () async {
      ...
      when(mockTUserRepository.findByPrimaryKey(any))
      ...
      when(mockTUserProfileRepository.findByPrimaryKey(any))
    ...
    test(
      ...
      () async {
        when(mockAuthRepository.currentUser).thenReturn(mockUser);
        when(mockUser.id).thenReturn(userId);
        when(mockTUserRepository.findByPrimaryKey(userId))
            .thenAnswer((_) async => tUser);
        when(
          mockTUserRepository.updateByPrimaryKey(
            id: userId,
    ...
    test('ユーザ名のみ更新する場合、TUser 関連とストレージ関連の処理は行われないこと', () async {
      when(mockAuthRepository.currentUser).thenReturn(mockUser);
      when(mockUser.id).thenReturn(userId);
      when(mockTUserProfileRepository.findByPrimaryKey(userId))
          .thenAnswer((_) async => tUserProfile);
      when(
        mockTUserProfileRepository.updateByPrimaryKeySelective(
          userId: userId,
    ...
    test(
      ...
      () async {
        ...
        when(mockTUserProfileRepository.findByPrimaryKey(userId))
            .thenAnswer((_) async => tUserProfile.copyWith(
                  avatarUrl: null,
                ));
        when(
          mockTUserProfileRepository.updateByPrimaryKeySelective(
