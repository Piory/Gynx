...
void main() {
  ...
  group('正常系', () {
    test('渡されたユーザーID が FindUserUseCase#execute に渡されていること', () async {
      when(mockFindUserUseCase.execute(vUser.userId)).thenAnswer((_) async => vUser);
      final result = await createContainer().read(userNotifierProvider(vUser.userId).future);
      expect(result, vUser);
