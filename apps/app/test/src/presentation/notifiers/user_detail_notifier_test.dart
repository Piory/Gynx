...
import 'package:app/src/presentation/notifiers/user_detail_notifier.dart';
...
void main() {
  ...
  group('正常系', () {
    test('渡されたユーザーID が FindUserDetailUseCase#execute に渡されていること', () async {
      when(mockFindUserDetailUseCase.execute(vUserDetail.userId)).thenAnswer((_) async => vUserDetail);
      final result = await createContainer().read(userDetailNotifierProvider(vUserDetail.userId).future);
